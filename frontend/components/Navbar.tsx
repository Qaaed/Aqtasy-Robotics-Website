...
const Navbar: React.FC = () => {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-shadow duration-300 bg-white/90 dark:bg-[#18181B]/90 backdrop-blur-lg border-b border-gray-200 dark:border-gray-800 ${navClass}`}>
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-24">
          <div className="flex-shrink-0">
            <a href="#home" onClick={(e) => handleNavClick(e, '#home')} className="flex items-center space-x-2 text-[#3A2F5A] transition-transform duration-300 hover:scale-105">
              <LogoIcon className="h-10 w-10" />
              <span className="font-bold text-2xl text-[#1A1A1A] dark:text-[#E5E5E5]">Aphasia Robot</span>
            </a>
          </div>
      ...
      {/* Mobile Menu */}
      <div className={`${isOpen ? 'block' : 'hidden'} md:hidden bg-white dark:bg-[#18181B] border-t border-gray-200 dark:border-gray-700`}>
