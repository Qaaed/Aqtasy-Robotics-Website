...
const TechCard = ({ softwareName, hardwareName, isFlipped }) => {
    return (
        <div className="aspect-[3/2] [perspective:1000px]">
            ...
            >
                ...
                <div className="absolute w-full h-full [backface-visibility:hidden] bg-white dark:bg-[#1E1E2F] flex items-center justify-center text-center p-4 rounded-xl border border-gray-200/80 dark:border-gray-800/80 shadow-lg">
                    <p className="font-semibold text-[#3A2F5A] dark:text-gray-300">{softwareName}</p>
                </div>
                ...
                <div className="absolute w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] bg-white dark:bg-[#1E1E2F] flex items-center justify-center text-center p-4 rounded-xl border border-gray-200/80 dark:border-gray-800/80 shadow-lg">
                     <p className="font-semibold text-[#3A2F5A] dark:text-gray-300">{hardwareName}</p>
                </div>
            </div>
        </div>
    );
    ...
    <section id="tech" className="py-20 sm:py-32">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <AnimatedSection className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight">Our Technology Stack</h2>
        ...
        <AnimatedSection>
            <div className="flex justify-center items-center gap-4 mb-12 p-2 bg-gray-100 dark:bg-gray-800/50 rounded-full w-fit mx-auto">
                <button
                    ...
                    className={`px-6 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${activeTab === 'software' ? 'bg-white dark:bg-[#3A2F5A] text-[#3A2F5A] dark:text-white shadow-md' : 'text-[#6B7280] dark:text-[#9CA3AF]'}`}
                >
                    Software
                </button>
                <button
                    onClick={() => setActiveTab('hardware')}
                    className={`px-6 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${activeTab === 'hardware' ? 'bg-white dark:bg-[#3A2F5A] text-[#3A2F5A] dark:text-white shadow-md' : 'text-[#6B7280] dark:text-[#9CA3AF]'}`}
                >
                    Hardware
                </button>
            </div>
